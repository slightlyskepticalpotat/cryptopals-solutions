# Modified from https://github.com/FiloSottile/crypto.py/blob/master/3/md4.py
...
class MD4():
    ...
    def __init__(self, message, regs, length):
        self.A, self.B, self.C, self.D = regs

        if not length:
            length = len(message)
        length = struct.pack('<Q', length * 8)
